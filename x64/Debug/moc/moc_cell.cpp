/****************************************************************************
** Meta object code from reading C++ file 'cell.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../../../cell.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'cell.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_cell_t {
    QByteArrayData data[9];
    char stringdata0[111];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_cell_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_cell_t qt_meta_stringdata_cell = {
    {
QT_MOC_LITERAL(0, 0, 4), // "cell"
QT_MOC_LITERAL(1, 5, 20), // "sendDeleteCellSignal"
QT_MOC_LITERAL(2, 26, 0), // ""
QT_MOC_LITERAL(3, 27, 7), // "cellNum"
QT_MOC_LITERAL(4, 35, 16), // "sendDisplayImage"
QT_MOC_LITERAL(5, 52, 15), // "sendUploadClick"
QT_MOC_LITERAL(6, 68, 13), // "onDeleteClick"
QT_MOC_LITERAL(7, 82, 14), // "onDisplayClick"
QT_MOC_LITERAL(8, 97, 13) // "onUploadClick"

    },
    "cell\0sendDeleteCellSignal\0\0cellNum\0"
    "sendDisplayImage\0sendUploadClick\0"
    "onDeleteClick\0onDisplayClick\0onUploadClick"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_cell[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   44,    2, 0x06 /* Public */,
       4,    1,   47,    2, 0x06 /* Public */,
       5,    1,   50,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       6,    0,   53,    2, 0x0a /* Public */,
       7,    0,   54,    2, 0x0a /* Public */,
       8,    0,   55,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::Int,    2,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void cell::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<cell *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->sendDeleteCellSignal((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->sendDisplayImage((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->sendUploadClick((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->onDeleteClick(); break;
        case 4: _t->onDisplayClick(); break;
        case 5: _t->onUploadClick(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (cell::*)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&cell::sendDeleteCellSignal)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (cell::*)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&cell::sendDisplayImage)) {
                *result = 1;
                return;
            }
        }
        {
            using _t = void (cell::*)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&cell::sendUploadClick)) {
                *result = 2;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject cell::staticMetaObject = { {
    QMetaObject::SuperData::link<QMainWindow::staticMetaObject>(),
    qt_meta_stringdata_cell.data,
    qt_meta_data_cell,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *cell::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *cell::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_cell.stringdata0))
        return static_cast<void*>(this);
    return QMainWindow::qt_metacast(_clname);
}

int cell::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}

// SIGNAL 0
void cell::sendDeleteCellSignal(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void cell::sendDisplayImage(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void cell::sendUploadClick(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
